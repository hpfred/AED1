...
bool inserir(Dicionario * dicionario, char * pais, char * frase) {
    ...
    if(dicionario->letras[LetraAtual].pais == NULL){
        dicionario->letras[LetraAtual].pais = malloc(sizeof(char)*strlen(pais)+1);
        dicionario->letras[LetraAtual].frase = malloc(sizeof(char)*strlen(frase)+1);

        strcpy(dicionario->letras[LetraAtual].pais, pais);
        strcpy(dicionario->letras[LetraAtual].frase, frase);
        return true;
    }
...
Dicionario * initDicionario() {
    ...
    for (int i = 0; i < ALFABETO; i++) {
        ...
        d->letras[i].frase = NULL;              /// Programa original modificado, pois a função 'pesquisar()' tem IF com dicionario->letras[i].frase não inicializado
    }
...
int main() {
    ...
    /// Programa original modificado, para dar 'free' nos espaços de memória que foram alocados
    for(int i = 0; i < ALFABETO; i++){
